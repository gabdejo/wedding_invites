...
export default function RegistrySection({ content }: Props) {
  return (
    <section className="bg-[#faf8f4] px-6 py-24 text-center" id="registry">
        ...
        <div className="flex justify-center gap-4">
          {content.stores.map(({ name, url }) => (
            <a
              key={name}
              href={url}
              className="border border-[#2c2c2c] px-8 py-3 text-sm uppercase tracking-[0.2em] text-[#2c2c2c] transition-colors hover:bg-[#2c2c2c] hover:text-white"
